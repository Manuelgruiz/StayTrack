...
async def gw_get_user(user_id: int, auth=Depends(auth_user)):
    ...
    async with httpx.AsyncClient(follow_redirects=True, timeout=30.0) as c:
        r = await c.get(f"{USER_SVC}/v1/users/{user_id}")
        if r.is_error:
            try:
                raise HTTPException(status_code=r.status_code, detail=r.json())
            except Exception:
...
async def gw_update_user(user_id: int, body: UserCreate, auth=Depends(auth_user)):
    ...
    async with httpx.AsyncClient(follow_redirects=True, timeout=30.0) as c:
        r = await c.put(f"{USER_SVC}/v1/users/{user_id}", json=body.model_dump())
        if r.is_error:
            try:
                raise HTTPException(status_code=r.status_code, detail=r.json())
            except Exception:
...
async def gw_add_food(user_id: int, body: FoodLogCreate, auth=Depends(auth_user)):
    ...
    async with httpx.AsyncClient(follow_redirects=True, timeout=30.0) as c:
        rf = await c.post(f"{TRACKER_SVC}/v1/users/{user_id}/foods", json=body.model_dump(mode='json'))
        if rf.is_error:
            try:
                raise HTTPException(status_code=rf.status_code, detail=rf.json())
            except Exception:
                raise HTTPException(status_code=rf.status_code, detail=rf.text)
        return rf.json()
...
async def gw_list_foods(user_id: int, limit: int = Query(50, le=200), offset: int = 0, auth=Depends(auth_user)):
    async with httpx.AsyncClient(follow_redirects=True, timeout=30.0) as c:
        r = await c.get(f"{TRACKER_SVC}/v1/users/{user_id}/foods", params={"limit":limit,"offset":offset})
        if r.is_error:
            try:
                raise HTTPException(status_code=r.status_code, detail=r.json())
            except Exception:
...
async def gw_add_exercise(user_id: int, body: ExerciseLogCreate, auth=Depends(auth_user)):
    ...
    async with httpx.AsyncClient(follow_redirects=True, timeout=30.0) as c:
        r = await c.post(f"{TRACKER_SVC}/v1/users/{user_id}/exercises", json=body.model_dump(mode='json'))
        if r.is_error:
            try:
                raise HTTPException(status_code=r.status_code, detail=r.json())
            except Exception:
...
async def gw_list_exercises(user_id: int, limit: int = Query(50, le=200), offset: int = 0, auth=Depends(auth_user)):
    ...
    async with httpx.AsyncClient(follow_redirects=True, timeout=30.0) as c:
        r = await c.get(f"{TRACKER_SVC}/v1/users/{user_id}/exercises", params={"limit":limit,"offset":offset})
        if r.is_error:
            try:
                raise HTTPException(status_code=r.status_code, detail=r.json())
            except Exception:
